...
from lora_utils import adalora
...
from .configuration_t5_lora import T5Config
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        # LoRA Linear layers
        if  config.lora_mode == 'LoRA':
            self.q = lora.Linear(self.d_model, self.inner_dim, r=config.r, lora_alpha= config.lora_alpha,
                                 lora_dropout=config.lora_dropout, bias=False)
            self.k = lora.Linear(self.d_model, self.inner_dim, r=config.r, lora_alpha= config.lora_alpha,
                                 lora_dropout=config.lora_dropout, bias=False)
            self.v = lora.Linear(self.d_model, self.inner_dim, r=config.r, lora_alpha= config.lora_alpha,
                                 lora_dropout=config.lora_dropout, bias=False)
