...
import sys
sys.path.append("/hy-tmp/moment")
sys.path.append("/hy-tmp/moment/momentfm_hand")
import torch
import json
from huggingface_hub import PyTorchModelHubMixin
from torch import nn
from examples.src.transformers.models.t5_v2 import T5Config, T5EncoderModel, T5Model
from momentfm_hand.common import TASKS
from momentfm_hand.data.base import TimeseriesOutputs
from momentfm_hand.models.layers.embed import PatchEmbedding, Patching
from momentfm_hand.models.layers.revin import RevIN
from momentfm_hand.utils.masking import Masking
from momentfm_hand.utils.utils import (
...
class MOMENT_LORA(nn.Module):
    ...
    def _get_transformer_backbone(self, config) -> nn.Module:
        if config.getattr("randomly_initialize_backbone", False):
            ...
            logging.info(
                f"Initializing transformer with LoRA."
            )
...
class MOMENTPipeline_LORA(MOMENT_LORA, PyTorchModelHubMixin):
    def __init__(self, config: Namespace | dict, **kwargs: dict):
...
# if __name__ == '__main__':
    # model = MOMENTPipeline_LORA.from_pretrained(
